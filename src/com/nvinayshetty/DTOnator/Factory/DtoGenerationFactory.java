...
import com.nvinayshetty.DTOnator.ClassAdder.AddToPackage;
...
public class DtoGenerationFactory {
    public static Void getDtoGeneratorFor(FeedType type, Project project, PsiFile psiFile, JSONObject validFeed, PsiClass psiClass) {
        switch (type) {
            case JsonObject:
                new JsonDtoGenerator(project, psiFile, validFeed, psiClass, new SimpleFieldCreator(), new AddToPackage(psiFile.getContainingDirectory())).execute();
        }
        return null;
    }
}
